...
Filtering reads (maxEE ≈ 1 error/200 bp sequence is good starting point for this amplicons)
...
```{r, warning=FALSE, message=FALSE, size="tiny"}
#pseq <- subset_taxa(pseq, Kingdom == c("Bacteria", "Archaea"), na.rm = TRUE)
```
...
#prune non-bacterial taxa
#pseq_vsearch97 <- subset_taxa(pseq_vsearch97, Kingdom == c("Bacteria", "Archaea"))
```
...
#prune non-bacterial taxa
#pseq_vsearch99 <- subset_taxa(pseq_vsearch99, Kingdom == c("Bacteria", "Archaea"))
```
...
Low bacterial diversity in the samples is most likely explanation why denoising seemingly produces good results for long 16S rRNA. Error rate plot looks in this case flawless. However, it is notable that all samples contain over 150 k unique reads.

On the other hand, vsearch produced very high number of variants, over 900 and 9000, respectively.
