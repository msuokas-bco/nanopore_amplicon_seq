...
Difference between methods remains, but in much smaller extent
...
Then, pick five most abundant features
...
For stacked barplots, assay data is melted
...
In sample set, we evidently have pure microbial cultures, each method gave exactly same result. Low diversity might explain why dada2 error profile looks very clean.

Clustering of reads produce lot more low abundant noise, but it doesn't show in the end results (with two digits accuracy). There are two discrepancies in 10 most commond features between denoising and clustering. Dada picked up fonticella and clustering methods acidithiobacillus.
